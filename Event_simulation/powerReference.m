function [Pref, Pref_pu, Qref] = powerReference(Snom,fpRange,normalOperation,potMin)
...
% 	value while Qreg assumes values from 10% to 90% of Snom
    ...
    % potmin: minimum power level in pre-power-down operation 
    
    operation = rand;
    if operation < normalOperation
        fp = ramdomValue(fpRange(1),fpRange(2));
        Soperational = Snom*ramdomValue(potMin,1);
        Pref = Soperational*fp;
        Pref_pu = Pref/Snom; 
        Qref = -Soperational*sin(acos(fp));
    else
        Pref_pu = 0.02;   % minimum value to run the power flow
        Pref = Pref_pu*Snom;
